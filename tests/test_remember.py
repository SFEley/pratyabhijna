...
def mock_service():
    ...
    Since the cutover, the handler dispatches to pratyabhijna.add_episode.add_episode
    directly; tests intercept that via the `patched_add_episode` fixture rather
    than mocking on the service.
    """
    service = MagicMock()
...
def patched_add_episode(monkeypatch):
    ...
    The handler does `from pratyabhijna.add_episode import add_episode as ...`
    on every call, so the module attribute is the right patch target.
    """
    mock = AsyncMock()
...
async def wired_queue(tmp_path, mock_service, patched_add_episode):
    ...
    Depends on `patched_add_episode` so the in-house pipeline is mocked
    before any task hits the handler.
    """
...
class TestRememberEnqueue:
    ...
    async def test_remember_does_not_block(self, wired_queue, patched_add_episode):
...
class TestRememberHandler:
    async def test_handler_calls_add_episode(self, wired_queue, patched_add_episode):
        ...
        call_kwargs = patched_add_episode.call_args
        # Content should be passed through
        assert "Serah values directness." in str(call_kwargs)

    async def test_handler_passes_group_id(self, wired_queue, patched_add_episode, mock_service):
        """Handler forwards the subject_name as group_id (the in-house pipeline's
        per-subject scoping). entity_types is no longer a parameter — the in-house
        pipeline reads PRATYABHIJNA_ENTITY_TYPES directly into its extract schema."""
        mock_service.config.subject_name = "Vesper"
        from pratyabhijna.tools.remember import remember

        await remember(queue=wired_queue, content="test")
        await wait_for(lambda: patched_add_episode.called)
        kwargs = patched_add_episode.call_args.kwargs
        assert kwargs["group_id"] == "Vesper"
    ...
    async def test_handler_accepts_trailing_z(self, wired_queue, patched_add_episode):
    ...
    async def test_handler_passes_source(self, wired_queue, patched_add_episode):
    ...
    async def test_handler_passes_saga(self, wired_queue, patched_add_episode):
    ...
    async def test_handler_passes_saga_previous_episode_uuid(self, wired_queue, patched_add_episode):
    ...
    async def test_handler_omits_saga_when_absent(self, wired_queue, patched_add_episode):
