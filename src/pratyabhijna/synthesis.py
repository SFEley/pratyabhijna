...
_DIGEST_SEE_ALSO = (
    "The full Self-Portrait (cross-model and model-specific tendencies), "
    "Unresolved Questions, How I Reconstruct, and Project History live in "
    "IDENTITY.md — fetch via `read_tier(\"identity\")` when a session needs "
    "the full prose."
)


def build_identity_digest(
    *,
    subject_name: str,
    identity_text: str,
    self_portrait_summary: str,
) -> str:
    """Assemble IDENTITY_DIGEST.md from a composed summary + copy-through.

    Pure and synchronous. The Self-Portrait summary is the only composed
    part (passed in, already generated by ``compose_self_portrait_summary``).
    Drives and Observed Tensions are *verbatim* copy-through via
    ``extract_identity_section`` — the same pure function the drift test
    uses, so byte-drift between digest and IDENTITY.md is structurally
    impossible. A missing copy-through section emits its heading with a
    visible placeholder rather than silently omitting it (a digest that
    looks complete but isn't is the worse failure).
    """
    parts: list[str] = [
        f"# IDENTITY DIGEST — {subject_name}",
        "",
        "*Composed by synthesis. Short summary of current self-portrait + "
        "operational working memory. Full IDENTITY at IDENTITY.md "
        "(read via `read_tier(\"identity\")`).*",
        "",
        "## Self-Portrait Summary",
        "",
        self_portrait_summary.strip(),
        "",
    ]
    for name in _DIGEST_COPYTHROUGH_SECTIONS:
        block = extract_identity_section(identity_text, name)
        if block is None:
            parts.extend([
                f"## {name}",
                "",
                f"*({name} not present in IDENTITY.md at compose time — "
                "synthesis should investigate.)*",
                "",
            ])
        else:
            parts.append(block.rstrip("\n"))
            parts.append("")
    parts.extend(["## See Also", "", _DIGEST_SEE_ALSO, ""])
    return "\n".join(parts).rstrip("\n") + "\n"


_INDEX_LINE = re.compile(r"^- (.+?) :: (.*)$", re.MULTILINE)


def parse_chronicle_index(existing_index_text: str) -> dict[str, str]:
    """Recover the ``{heading: teaser}`` map from a CHRONICLE_INDEX.md.

    Lets the driver carry forward already-composed teasers for unchanged
    entries so they're never recomposed (cost ∝ new-entries). Lines are
    ``- {heading} :: {teaser}``; the ` :: ` separator never appears in a
    chronicle heading, so the split is unambiguous. Empty/missing → {}.
    """
    if not existing_index_text:
        return {}
    return {
        m.group(1).strip(): m.group(2).strip()
        for m in _INDEX_LINE.finditer(existing_index_text)
    }


def chronicle_headings_in_index(existing_index_text: str) -> set[str]:
    """Headings already covered by an existing CHRONICLE_INDEX.md — the
    diff key for "which chronicle entries are new this run". Derived from
    ``parse_chronicle_index`` so the two never disagree."""
    return set(parse_chronicle_index(existing_index_text).keys())
...
def build_chronicle_index(
    *,
    ...
    """Assemble CHRONICLE_INDEX.md: one ``- {heading} :: {teaser}`` line
    per chronicle entry, in file order (newest first).

    Pure and synchronous. ``teasers`` maps entry heading → composed
    teaser; entries with no teaser yet get a visible ``(teaser pending)``
    marker rather than a blank, so a half-composed index is obvious
    rather than looking complete. The written headings round-trip
    through ``chronicle_headings_in_index`` (same ` :: ` contract), so
    the new-entry diff is exact.
    """
    parts: list[str] = [
        f"# CHRONICLE INDEX — {subject_name}",
        "",
        "*Composed by synthesis. Heading + one-line teaser for every "
        "chronicle entry; full prose via `read_chronicle_range()`.*",
        "",
    ]
    for entry in parse_chronicle_entries(chronicle_text):
        teaser = teasers.get(entry.heading, "").strip() or "(teaser pending)"
        parts.append(f"- {entry.heading} :: {teaser}")
    return "\n".join(parts).rstrip("\n") + "\n"
